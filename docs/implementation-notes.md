...
- 筆記本詳情預設顯示「文件」，右側同層分頁可切換至「知識圖譜」；分頁狀態只保留在目前頁面，不寫入網址或瀏覽器儲存空間。
- 筆記本圖譜與知識庫圖譜共用 `src/components/KnowledgeGraphCanvas.vue`（力導向佈局、群組色、搜尋與類型篩選、節點詳情互動），兩邊只各自準備節點資料與詳情面板內容。
...
- 「文件管理」是文件的唯一入口，分成「全部文件」（清單、篩選、批次核准與批次重新處理）、「需要處理」（失敗、部分附件失敗、停滯與策略待套用的工作）與「處理策略」（全域預設及檔案類型層）三個分頁；個別文件覆蓋及切塊內容編輯仍由文件詳情負責。
- 原本的 `/admin/processing` 已併入 `/admin/documents`，保留為轉導路由：`tab=attention`、`tab=strategy` 原樣帶入，其餘（含舊的 `tab=all`）一律進「全部文件」分頁。
...
- 員工前台 `/ask` 不提供過度寬泛的「全公司知識」、模型一般知識或網路搜尋，預設使用「公司制度」；公司知識庫與個人筆記本會在同一個彈窗漸進展開「限定文件（選填）」。後台短效 AI 小幫手仍沿用自己的來源與網路搜尋選項，不受此限制。
